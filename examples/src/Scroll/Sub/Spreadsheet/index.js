...
{
	if (region.bo.aH === region.bC.aH)
	{
		return 'on line ' + region.bo.aH;
	}
	return 'on lines ' + region.bo.aH + ' through ' + region.bC.aH;
}
...
var $author$project$Scroll$Sub$Spreadsheet$Main$init = _Utils_Tuple2(
	{ap: $author$project$Scroll$Engine$Sub$init, aJ: $author$project$Scroll$Sub$Spreadsheet$Main$Idle},
							...
							A3(
								...
								F2(
									function (viewport, containerEl) {
										return {aT: containerEl, a4: $elm$core$Maybe$Nothing, s: viewport};
									}),
								$author$project$Scroll$Internal$Shared$Dom$getViewport(container),
								$author$project$Scroll$Internal$Shared$Dom$getContainerInfo(container)));
					}
		...
		return _Utils_update(
			config,
			{
				a: _Utils_update(
					targetData,
					{
						aI: _Utils_Tuple2(offsetX, offsetY)
					})
			});
	});
...
var $author$project$Scroll$Internal$Engine$Sub$createScrollAnimationConfig = F2(
	function (scrollBuilder, scrollTarget) {
		return {
			...
			aq: $author$project$Scroll$Internal$Shared$ScrollTarget$getTargetX(scrollTarget),
			ar: $author$project$Scroll$Internal$Shared$ScrollTarget$getTargetY(scrollTarget),
...
var $author$project$Scroll$Internal$Engine$Sub$createScrollAnimationFromDom = F4(
	function (scrollBuilder, scrollTarget, domResult, element) {
		...
		var elementContentPosition = function () {
			var _v3 = domResult.aT;
			if (!_v3.$) {
				var containerEl = _v3.a;
				return {d: ((element.ax.d - containerEl.ax.d) + viewport.s.d) - offsetX, e: ((element.ax.e - containerEl.ax.e) + viewport.s.e) - offsetY};
			} else {
				return {d: (element.ax.d + viewport.s.d) - offsetX, e: (element.ax.e + viewport.s.e) - offsetY};
			}
		}();
		...
		var config = _Utils_update(
			baseConfig,
			{aq: clampedTarget.d, ar: clampedTarget.e});
		...
		return {c: config, F: startPosition.d, G: startPosition.e, ah: delayMs === 0.0, aw: delayMs, aG: actualDuration, A: 0.0, t: false, P: 0.0, C: startPosition.d, D: startPosition.e};
	});
...
var $author$project$Scroll$Internal$Engine$Sub$updateScrollAnimation = F2(
	function (deltaMs, animation) {
		if (!animation.ah) {
			...
			return (_Utils_cmp(newElapsedMs, animation.aw) > -1) ? _Utils_update(
				animation,
			...
			var newElapsedMs = animation.A + deltaMs;
			var progress = A2($elm$core$Basics$min, 1.0, newElapsedMs / animation.aG);
			...
			var newCurrentX = function () {
				...
				if (_v1 === 1) {
					...
					return animation.C + ((animation.c.aq - animation.C) * easedProgress);
				}
			}();
			var newCurrentY = function () {
				...
				if (!_v0) {
					...
					return animation.D + ((animation.c.ar - animation.D) * easedProgress);
				}
			}();
			return _Utils_update(
				animation,
				{F: newCurrentX, G: newCurrentY, A: newElapsedMs, P: progress});
		}
	});
	...
	function (fromInternalEvent, toMsg, msg, _v0) {
		...
		switch (msg.$) {
			...
			case 1:
				...
				var hasDistance = A5($author$project$Scroll$Internal$Engine$Sub$calculateDistance, scrollAnimation.c.ad, scrollAnimation.C, scrollAnimation.D, scrollAnimation.c.aq, scrollAnimation.c.ar) > 0;
		...
		if (!msg.$) {
			...
			return _Utils_Tuple2(
				_Utils_update(
					...
					{
						ap: newScrollState,
						aJ: A3($elm$core$List$foldl, $author$project$Scroll$Sub$Spreadsheet$Main$applyEvent, model.aJ, events)
					}),
				scrollCmd);
		}
	});
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				$author$project$Scroll$Sub$Spreadsheet$Main$NavigateTo(region.W)),
				$elm$html$Html$Attributes$class('ui-action-button'),
				A2($elm$html$Html$Attributes$style, 'background-color', region.S)
			]),
		_List_fromArray(
			[
				$elm$html$Html$text(region.V + (' ' + region.X))
			]));
};
var $author$project$Scroll$Sub$Spreadsheet$Main$regions = _List_fromArray(
	[
		{af: 4, S: '#3b6fa0', V: '💰', W: 'region-revenue', X: 'Revenue', ao: 5},
		{af: 10, S: '#a03b3b', V: '📉', W: 'region-expenses', X: 'Expenses', ao: 13},
		{af: 7, S: '#3b8050', V: '📈', W: 'region-forecast', X: 'Forecast', ao: 22},
		{af: 13, S: '#7a6b30', V: '📊', W: 'region-summary', X: 'Summary', ao: 30},
		{af: 16, S: '#6b3a8b', V: '🚀', W: 'region-growth', X: 'YoY Growth', ao: 35}
	]);
	...
	function (row, col) {
		...
		if (!_v0.$) {
			...
			return A2(
				...
				_List_fromArray(
					[
						$elm$html$Html$Attributes$id(region.W),
						A2($elm$html$Html$Attributes$style, 'background-color', region.S),
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				$author$project$Scroll$Sub$Spreadsheet$Main$statusBar(model.aJ),
...
var $author$project$Scroll$Sub$Spreadsheet$Main$main = $elm$browser$Browser$element(
	{
		dj: function (_v0) {
			return $author$project$Scroll$Sub$Spreadsheet$Main$init;
		},
