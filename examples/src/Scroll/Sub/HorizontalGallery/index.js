...
{
	if (region.bn.aG === region.bB.aG)
	{
		return 'on line ' + region.bn.aG;
	}
	return 'on lines ' + region.bn.aG + ' through ' + region.bB.aG;
}
...
var $author$project$Scroll$Sub$HorizontalGallery$Main$init = _Utils_Tuple2(
	{ao: $author$project$Scroll$Engine$Sub$init, aI: $author$project$Scroll$Sub$HorizontalGallery$Main$Idle},
							...
							A3(
								...
								F2(
									function (viewport, containerEl) {
										return {aS: containerEl, a3: $elm$core$Maybe$Nothing, s: viewport};
									}),
								$author$project$Scroll$Internal$Shared$Dom$getViewport(container),
								$author$project$Scroll$Internal$Shared$Dom$getContainerInfo(container)));
					}
...
var $author$project$Scroll$Internal$Engine$Sub$createScrollAnimationConfig = F2(
	function (scrollBuilder, scrollTarget) {
		return {
			...
			ap: $author$project$Scroll$Internal$Shared$ScrollTarget$getTargetX(scrollTarget),
			aq: $author$project$Scroll$Internal$Shared$ScrollTarget$getTargetY(scrollTarget),
	...
	function (scrollBuilder, scrollTarget, domResult, element) {
		...
		var elementContentPosition = function () {
			...
			if (!_v3.$) {
				...
				return {d: ((element.aw.d - containerEl.aw.d) + viewport.s.d) - offsetX, e: ((element.aw.e - containerEl.aw.e) + viewport.s.e) - offsetY};
			} else {
				return {d: (element.aw.d + viewport.s.d) - offsetX, e: (element.aw.e + viewport.s.e) - offsetY};
			}
		}();
		...
		var config = _Utils_update(
			baseConfig,
			{ap: clampedTarget.d, aq: clampedTarget.e});
		...
		return {c: config, G: startPosition.d, H: startPosition.e, ah: delayMs === 0.0, av: delayMs, aF: actualDuration, A: 0.0, t: false, U: 0.0, C: startPosition.d, D: startPosition.e};
	});
...
var $author$project$Scroll$Internal$Engine$Sub$updateScrollAnimation = F2(
	function (deltaMs, animation) {
		if (!animation.ah) {
			...
			return (_Utils_cmp(newElapsedMs, animation.av) > -1) ? _Utils_update(
				animation,
				{ah: true, A: 0.0}) : _Utils_update(
				animation,
				{A: newElapsedMs});
		} else {
			var newElapsedMs = animation.A + deltaMs;
			var progress = A2($elm$core$Basics$min, 1.0, newElapsedMs / animation.aF);
			...
			var newCurrentX = function () {
				...
				if (_v1 === 1) {
					...
					return animation.C + ((animation.c.ap - animation.C) * easedProgress);
				}
			}();
			var newCurrentY = function () {
				...
				if (!_v0) {
					...
					return animation.D + ((animation.c.aq - animation.D) * easedProgress);
				}
			}();
			return _Utils_update(
				animation,
				{G: newCurrentX, H: newCurrentY, A: newElapsedMs, U: progress});
		}
	});
	...
	function (fromInternalEvent, toMsg, msg, _v0) {
		...
		switch (msg.$) {
						...
						function (scrollId, scrollAnim, _v3) {
							...
							if (scrollAnim.t) {
								...
								var scrollCmd = A2(
									...
									A3($author$project$Scroll$Internal$Shared$Dom$setViewport, updatedScroll.c.h, updatedScroll.G, updatedScroll.H));
								...
								var progressEvent = A3(
									...
									cid,
									{d: updatedScroll.G, e: updatedScroll.H},
									updatedScroll.U);
								if (updatedScroll.U < 1.0) {
									return _Utils_Tuple3(
			...
			case 1:
				...
				var hasDistance = A5($author$project$Scroll$Internal$Engine$Sub$calculateDistance, scrollAnimation.c.ae, scrollAnimation.C, scrollAnimation.D, scrollAnimation.c.ap, scrollAnimation.c.aq) > 0;
		...
		if (!msg.$) {
			...
			return _Utils_Tuple2(
				_Utils_update(
					...
					{
						ao: newScrollState,
						aI: A3($elm$core$List$foldl, $author$project$Scroll$Sub$HorizontalGallery$Main$applyEvent, model.aI, events)
					}),
				scrollCmd);
		}
	});
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				$author$project$Scroll$Sub$HorizontalGallery$Main$ScrollTo(photo.J)),
				$elm$html$Html$Attributes$class('ui-action-button'),
				A2($elm$html$Html$Attributes$style, 'background-color', photo.F)
			]),
		_List_fromArray(
			[
				$elm$html$Html$text(photo.I + (' ' + photo.K))
			]));
};
var $author$project$Scroll$Sub$HorizontalGallery$Main$photos = _List_fromArray(
	[
		{F: '#4a6f8a', I: '🏔️', J: 'photo-mountains', K: 'Mountains'},
		{F: '#1a7a6e', I: '🌊', J: 'photo-ocean', K: 'Ocean'},
		{F: '#c47b3a', I: '🏜️', J: 'photo-desert', K: 'Desert'},
		{F: '#3a7a45', I: '🌲', J: 'photo-forest', K: 'Forest'},
		{F: '#5b7fa6', I: '🧊', J: 'photo-arctic', K: 'Arctic'},
		{F: '#8b3a3a', I: '🌋', J: 'photo-volcano', K: 'Volcano'},
		{F: '#8b7a3a', I: '🦁', J: 'photo-savanna', K: 'Savanna'},
		{F: '#2a7a8b', I: '🐠', J: 'photo-reef', K: 'Reef'}
	]);
...
var $author$project$Scroll$Sub$HorizontalGallery$Main$photoCard = function (photo) {
	return A2(
		...
		_List_fromArray(
			[
				$elm$html$Html$Attributes$id(photo.J),
				A2($elm$html$Html$Attributes$style, 'min-width', 'clamp(140px, 35vmin, 220px)'),
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						A2($elm$html$Html$Attributes$style, 'font-size', 'clamp(36px, 9vmin, 64px)')
					]),
				_List_fromArray(
					[
				...
				_List_fromArray(
					[
						A2($elm$html$Html$Attributes$style, 'font-size', 'clamp(13px, 2.2vmin, 18px)'),
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				$author$project$Scroll$Sub$HorizontalGallery$Main$statusBar(model.aI),
...
var $author$project$Scroll$Sub$HorizontalGallery$Main$main = $elm$browser$Browser$element(
	{
		dj: function (_v0) {
			return $author$project$Scroll$Sub$HorizontalGallery$Main$init;
		},
