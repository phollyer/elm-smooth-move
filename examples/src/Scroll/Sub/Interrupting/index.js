...
{
	if (region.bl.aE === region.bz.aE)
	{
		return 'on line ' + region.bl.aE;
	}
	return 'on lines ' + region.bl.aE + ' through ' + region.bz.aE;
}
...
var $author$project$Scroll$Sub$Interrupting$Main$Idle = 0;
...
var $author$project$Scroll$Sub$Interrupting$Main$init = _Utils_Tuple2(
	{ak: $author$project$Scroll$Engine$Sub$init, al: 0},
	$elm$core$Platform$Cmd$none);
var $author$project$Scroll$Sub$Interrupting$Main$GotScrollMsg = function (a) {
	return {$: 1, a: a};
};
...
var $author$project$Scroll$Sub$Interrupting$Main$subscriptions = function (model) {
	return A2($author$project$Scroll$Engine$Sub$subscriptions, $author$project$Scroll$Sub$Interrupting$Main$GotScrollMsg, model.ak);
};
var $author$project$Scroll$Sub$Interrupting$Main$Interrupted = 2;
var $author$project$Scroll$Sub$Interrupting$Main$Scrolling = 1;
var $author$project$Scroll$Sub$Interrupting$Main$Arrived = 3;
var $author$project$Scroll$Sub$Interrupting$Main$applyEvent = F2(
...
var $author$project$Scroll$Sub$Interrupting$Main$scrollToElement = function (targetId) {
	return A2(
		...
		A2(
			...
			A2(
				$elm$core$Basics$composeR,
				$author$project$Scroll$Builder$speed(120),
				A2(
					$elm$core$Basics$composeR,
					$author$project$Scroll$Builder$easing($author$project$Motion$Easing$Linear),
...
var $author$project$Scroll$Internal$Engine$Sub$createScrollAnimationConfig = F2(
	function (scrollBuilder, scrollTarget) {
		return {
			...
			am: $author$project$Scroll$Internal$Shared$ScrollTarget$getTargetX(scrollTarget),
			an: $author$project$Scroll$Internal$Shared$ScrollTarget$getTargetY(scrollTarget),
...
var $author$project$Scroll$Internal$Engine$Sub$createScrollAnimationFromDom = F4(
	function (scrollBuilder, scrollTarget, domResult, element) {
		...
		var config = _Utils_update(
			baseConfig,
			{am: clampedTarget.d, an: clampedTarget.e});
		...
		return {c: config, F: startPosition.d, G: startPosition.e, ad: delayMs === 0.0, at: delayMs, aD: actualDuration, A: 0.0, t: false, Q: 0.0, C: startPosition.d, D: startPosition.e};
	});
...
var $author$project$Scroll$Internal$Engine$Sub$updateScrollAnimation = F2(
	function (deltaMs, animation) {
		if (!animation.ad) {
			...
			var newElapsedMs = animation.A + deltaMs;
			var progress = A2($elm$core$Basics$min, 1.0, newElapsedMs / animation.aD);
			...
			var newCurrentX = function () {
				...
				if (_v1 === 1) {
					...
					return animation.C + ((animation.c.am - animation.C) * easedProgress);
				}
			}();
			var newCurrentY = function () {
				...
				if (!_v0) {
					...
					return animation.D + ((animation.c.an - animation.D) * easedProgress);
				}
			}();
			return _Utils_update(
				animation,
	...
	function (fromInternalEvent, toMsg, msg, _v0) {
		...
		switch (msg.$) {
			...
			case 1:
				...
				var hasDistance = A5($author$project$Scroll$Internal$Engine$Sub$calculateDistance, scrollAnimation.c.aa, scrollAnimation.C, scrollAnimation.D, scrollAnimation.c.am, scrollAnimation.c.an) > 0;
...
var $author$project$Scroll$Sub$Interrupting$Main$update = F2(
	function (msg, model) {
		if (!msg.$) {
			var targetId = msg.a;
			var redirecting = model.al === 1;
			var _v1 = A3(
				$author$project$Scroll$Engine$Sub$scroll,
				$author$project$Scroll$Sub$Interrupting$Main$GotScrollMsg,
				model.ak,
				$author$project$Scroll$Sub$Interrupting$Main$scrollToElement(targetId));
			...
			var scrollMsg = msg.a;
			var _v2 = A3($author$project$Scroll$Engine$Sub$update, $author$project$Scroll$Sub$Interrupting$Main$GotScrollMsg, scrollMsg, model.ak);
			...
			return _Utils_Tuple2(
				_Utils_update(
					...
					{
						ak: newScrollState,
						al: A3($elm$core$List$foldl, $author$project$Scroll$Sub$Interrupting$Main$applyEvent, model.al, events)
					}),
				scrollCmd);
		}
	});
var $author$project$Scroll$Sub$Interrupting$Main$ScrollTo = function (a) {
	return {$: 0, a: a};
};
...
var $author$project$Scroll$Sub$Interrupting$Main$filler = function (label) {
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
				A2($elm$html$Html$Attributes$style, 'padding', '16px'),
				A2($elm$html$Html$Attributes$style, 'background', '#f1f5f9'),
...
var $author$project$Scroll$Sub$Interrupting$Main$spacer = A2(
	$elm$html$Html$div,
	_List_fromArray(
		[
	...
	_List_Nil);
var $author$project$Scroll$Sub$Interrupting$Main$targetBlock = F3(
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					A2($elm$html$Html$Attributes$style, 'border-radius', '8px'),
					A2($elm$html$Html$Attributes$style, 'text-align', 'center'),
					A2($elm$html$Html$Attributes$style, 'font-size', 'clamp(16px, 3vmin, 24px)'),
...
var $author$project$Scroll$Sub$Interrupting$Main$scrollContent = A2(
	$elm$html$Html$div,
	_List_fromArray(
		[
	...
	_List_fromArray(
		[
			A3($author$project$Scroll$Sub$Interrupting$Main$targetBlock, 'top-target', '🟢 Top Target', '#22c55e'),
			$author$project$Scroll$Sub$Interrupting$Main$spacer,
			$author$project$Scroll$Sub$Interrupting$Main$filler('Scroll through me…'),
			$author$project$Scroll$Sub$Interrupting$Main$spacer,
			$author$project$Scroll$Sub$Interrupting$Main$filler('…keep scrolling…'),
			$author$project$Scroll$Sub$Interrupting$Main$spacer,
			A3($author$project$Scroll$Sub$Interrupting$Main$targetBlock, 'bottom-target', '🔴 Bottom Target', '#ef4444')
		]));
var $author$project$Scroll$Sub$Interrupting$Main$statusBar = function (status) {
	var _v0 = function () {
		switch (status) {
			case 0:
				return _Utils_Tuple2('#94a3b8', 'Idle — try clicking one button, then the other before it finishes');
			case 1:
				return _Utils_Tuple2('#3b82f6', 'Scrolling…');
			case 2:
				return _Utils_Tuple2('#a855f7', 'Redirected mid-flight — smoothly continuing to new target');
			default:
...
var $author$project$Scroll$Sub$Interrupting$Main$view = function (model) {
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
		...
		_List_fromArray(
			[
					...
					[
						...
						_List_fromArray(
							[
								...
								$author$project$Scroll$Sub$Interrupting$Main$ScrollTo('top-target')),
								$elm$html$Html$Attributes$class('ui-action-button primary')
							]),
						_List_fromArray(
							[
								$elm$html$Html$text('⬆️ Scroll to Top')
							])),
						A2(
						$elm$html$Html$button,
						_List_fromArray(
							[
								$elm$html$Html$Events$onClick(
								$author$project$Scroll$Sub$Interrupting$Main$ScrollTo('bottom-target')),
								$elm$html$Html$Attributes$class('ui-action-button warning')
							]),
						_List_fromArray(
							[
								$elm$html$Html$text('⬇️ Scroll to Bottom')
							]))
					])),
				$author$project$Scroll$Sub$Interrupting$Main$statusBar(model.al),
				A2(
				$elm$html$Html$div,
				_List_fromArray(
					[
				...
				_List_fromArray(
					[$author$project$Scroll$Sub$Interrupting$Main$scrollContent]))
			]));
};
var $author$project$Scroll$Sub$Interrupting$Main$main = $elm$browser$Browser$element(
	{
		dh: function (_v0) {
			return $author$project$Scroll$Sub$Interrupting$Main$init;
		},
		d3: $author$project$Scroll$Sub$Interrupting$Main$subscriptions,
		er: $author$project$Scroll$Sub$Interrupting$Main$update,
		es: $author$project$Scroll$Sub$Interrupting$Main$view
	});
_Platform_export({'Scroll':{'Sub':{'Interrupting':{'Main':{'init':$author$project$Scroll$Sub$Interrupting$Main$main(
