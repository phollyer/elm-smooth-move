...
var $author$project$Anim$Internal$Builder$getCurrentAnimationConfig = F2(
	function (animGroupName, _v0) {
		...
		return A2(
			...
			A2($author$project$Anim$Internal$Engine$Shared$AnimGroups$get, animGroupName, data.x.av));
	});
	...
	return wrapper(
		{
			...
			v: A3($author$project$Anim$Internal$Builder$resolveDelayWithDefault, config.v, globalData.aq, 0),
			cb: distance_,
...
var $author$project$Anim$Internal$Builder$initDefaults = {W: $author$project$Anim$Internal$Builder$CssUnitStore$empty, i: $author$project$Anim$Internal$Unit$emptyCssUnitAxes, aq: $elm$core$Maybe$Nothing, ar: $elm$core$Maybe$Nothing, U: $author$project$Anim$Internal$Unit$emptyCssUnitAxes, aC: $elm$core$Maybe$Nothing, ab: $elm$core$Maybe$Nothing, aD: $elm$core$Maybe$Nothing, aG: $elm$core$Maybe$Nothing, aJ: $elm$core$Maybe$Nothing, b$: $elm$core$Set$empty, au: $elm$core$Maybe$Nothing};
...
var $author$project$Anim$Internal$Builder$initState = {av: $author$project$Anim$Internal$Engine$Shared$AnimGroups$init, O: $author$project$Anim$Internal$Engine$Shared$AnimGroups$init, aX: $elm$core$Dict$empty, bU: $author$project$Anim$Internal$Engine$Shared$AnimGroups$init};
	...
	var resetInitEntryScope = function (_v0) {
		...
		return _Utils_update(
			data,
			{
				...
				c: _Utils_update(
					defs,
					{aG: $elm$core$Maybe$Nothing, aJ: $elm$core$Maybe$Nothing, au: $elm$core$Maybe$Nothing})
			});
	};
	var runInitEntry = F2(
		function (f, builder) {
...
var $author$project$Animation$ViewTimeline$Main$cards = _List_fromArray(
	[
		{cy: 'view-card-1', ax: 'Each card uses a ViewTimeline tied to itself as the scroll subject. As the card enters the viewport, it fades in and slides upward.', ay: '#6366f1', aE: '01', aK: 'Enter from below'},
		{cy: 'view-card-2', ax: 'Every card has its own ViewTimeline. Animations are fully independent - each one triggers only when that specific card enters the viewport.', ay: '#8b5cf6', aE: '02', aK: 'Independent timelines'},
		{cy: 'view-card-3', ax: 'The rangeStart and rangeEnd functions define exactly when during the card\u0027s lifecycle the animation plays - entry, cover, contain, or exit.', ay: '#a78bfa', aE: '03', aK: 'Range control'},
		{cy: 'view-card-4', ax: 'View timeline animations are fire-and-forget. No AnimState required - just trigger in your init and the browser handles the rest.', ay: '#7c3aed', aE: '04', aK: 'Fire and forget'},
		{cy: 'view-card-5', ax: 'Combine opacity and translate in a single pipeline to create polished reveal effects with minimal code.', ay: '#5b21b6', aE: '05', aK: 'Composable builders'},
		{cy: 'view-card-6', ax: 'All animations run via the Web Animations API on the compositor thread - no JavaScript timers, no Elm subscriptions, maximum performance.', ay: '#4c1d95', aE: '06', aK: 'WAAPI powered'}
	]);
					...
					[
						...
						_List_fromArray(
							[
								...
								A2($elm$html$Html$Attributes$style, 'color', card.ay),
